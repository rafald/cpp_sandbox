...
TO_UNSIGNED_LOW(int x) {
   return x;
}
static inline unsigned
TO_UNSIGNED_LOW(unsigned x) {
   return x;
}
static inline unsigned long long
TO_UNSIGNED_LOW(long long x) {
   return x;
}
static inline unsigned long long
TO_UNSIGNED_LOW(unsigned long long x) {
   return x;
}
// RLD TODO: NOT TESTED
static inline unsigned long
TO_UNSIGNED_LOW(long x) {
   return x;
}
static inline unsigned long
TO_UNSIGNED_LOW(unsigned long x) {
   return x;
}
#endif
...
TO_UNSIGNED_LOW(__int128_t x) {
   return x;
}
static inline __uint128_t
TO_UNSIGNED_LOW(__uint128_t x) {
   return x;
}
#else
#define TO_UNSIGNED_LOW(x)                                                          \
   (__builtin_choose_expr(                                                          \
       ...
       __builtin_choose_expr(sizeof(x) <= sizeof(long long),                        \
                             ...
                             __builtin_choose_expr(sizeof(x) <= sizeof(__int128_t), \
                                                   (__uint128_t)(x),                \
...
#define _IS_ADD_OVERFLOW_S(x, y, c)                                                    \
   ({                                                                                  \
      __typeof__(x) _x = (x), _y = (y), _z = (_x ^ ~_y) & (1 << (sizeof(_x) * 8 - 1)); \
      (int)(((_z & (((_x ^ _z) + _y + (c)) ^ ~_y)) >> (sizeof(_x) * 8 - 1)) & 1);      \
   })
...
#define is_add_overflow(x, y, c)                                                          \
   (sizeof(x) > MAX_INT_SIZE && !is_unsigned(x) ? _IS_ADD_OVERFLOW_S(x, y, c) : ({        \
      __typeof__(x) _x = (x), _y = (y), _s = (__typeof__(x))(to_unsigned(_x) + _y + (c)); \
      is_unsigned(_x) ? (int)((((_x & _y) | ((_x | _y) & ~_s)) >> (sizeof(_x) * 8 - 1)))  \
...
#define is_subtract_overflow(x, y, c)                                                     \
   (sizeof(x) > MAX_INT_SIZE && !is_unsigned(x) ? _IS_SUBTRACT_OVERFLOW_S(x, y, c) : ({   \
      __typeof__(x) _x = (x), _y = (y), _s = (__typeof__(x))(to_unsigned(_x) - _y - (c)); \
      is_unsigned(_x) ? (int)((((~_x & _y) | ((~_x | _y) & _s)) >> (sizeof(_x) * 8 - 1))) \
...
////////////////////////////////////////////////////////////////////////////////
// The following functions implement trapping arithmetic. These functions call the libc function abort upon signed arithmetic
// overflow.
...
__addvsi3(int a, int b) {
   TRACE2(a, b);
   if (is_add_overflow(a, b, 0)) ++int_add_overflow;
   return a + b;
}
extern "C" long
__addvdi3(long a, long b) {
   TRACE2(a, b);
   if (is_add_overflow(a, b, 0)) ++long_add_overflow;
   return a + b;
}
...
__subvsi3(int a, int b) {
   TRACE2(a, b);
   if (is_subtract_overflow(a, b, 0)) ++subtract_overflow;
   return a - b;
}
extern "C" long
