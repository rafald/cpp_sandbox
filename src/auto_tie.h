#pragma once
// https://github.com/jbandela/auto_tie/blob/master/include/auto_tie.hpp
...
struct auto_tie_helper {
   ...
   auto
   construct(T&& t, std::index_sequence<I...>) {
...
#define AUTO_TIE_IMPL(seq)                                                                \
   auto_tie([](BOOST_PP_SEQ_FOR_EACH_I(AUTO_TIE_HELPER1, _, seq)) {                       \
...
#define AUTO_TIE_REF_HELPER2(r, data, elem)                                                                                    \
   static_assert(!std::is_rvalue_reference<decltype(BOOST_PP_CAT(elem, data))>::value, "Cannot use AUTO_TIE_REF with rvalue"); \
   decltype(BOOST_PP_CAT(elem, data)) elem;

#define AUTO_TIE_REF_IMPL(seq)                                                            \
   auto_tie([](BOOST_PP_SEQ_FOR_EACH_I(AUTO_TIE_REF_HELPER1, _, seq)) {                   \
      struct f1f067cb_03fe_47dc_a56d_93407b318d12_auto_tie_struct {                       \
         BOOST_PP_SEQ_FOR_EACH(AUTO_TIE_REF_HELPER2, _, seq)                              \
      };                                                                                  \
      return static_cast<f1f067cb_03fe_47dc_a56d_93407b318d12_auto_tie_struct*>(nullptr); \
   })
