...
constexpr auto
distribution(Gen &g, T min, T max) {
...
struct Array2D {
   ...
   constexpr Data &
   operator()(const std::size_t col, const std::size_t row) {
      return m_data[col + (row * Cols)];
   }

   constexpr const Data &
   operator()(const std::size_t col, const std::size_t row) const {
      return m_data[col + (row * Cols)];
   }
...
struct Loc {
   ...
   constexpr Loc(const std::size_t t_col, const std::size_t t_row) : col(t_col), row(t_row) {}

   constexpr Loc() = default;
};

template <std::size_t num_cols, std::size_t num_rows>
constexpr Array2D<Cell, num_cols, num_rows>
make_maze() {
   PCG pcg;
   ...
   while (!history.empty()) {
      ...
      if (c > 0 && M(c - 1, r).visited == false) { check.emplace_back('L'); }
      if (r > 0 && M(c, r - 1).visited == false) { check.emplace_back('U'); }
      if (c < num_cols - 1 && M(c + 1, r).visited == false) { check.emplace_back('R'); }
      if (r < num_rows - 1 && M(c, r + 1).visited == false) { check.emplace_back('D'); }
...
template <std::size_t num_cols, std::size_t num_rows>
constexpr Array2D<WallType, num_cols * 2 + 1, num_rows * 2 + 1>
render_maze(const Array2D<Cell, num_cols, num_rows> &maze_data) {
   ...
   for (std::size_t col = 0; col < result.cols(); col += 2) {
      for (std::size_t row = 0; row < result.rows(); row += 2) {
         ...
         if (up && right && down && left) { result(col, row) = WallType::FourWay; }
         if (up && right && down && !left) { result(col, row) = WallType::RightTee; }
         if (up && right && !down && left) { result(col, row) = WallType::UpTee; }
         if (up && !right && down && left) { result(col, row) = WallType::LeftTee; }
         if (!up && right && down && left) { result(col, row) = WallType::DownTee; }

         if (up && right && !down && !left) { result(col, row) = WallType::LowerLeft; }
         if (up && !right && !down && left) { result(col, row) = WallType::LowerRight; }
         if (!up && !right && down && left) { result(col, row) = WallType::UpperRight; }
         if (!up && right && down && !left) { result(col, row) = WallType::UpperLeft; }
         if (!up && right && !down && left) { result(col, row) = WallType::Horizontal; }
         if (up && !right && down && !left) { result(col, row) = WallType::Vertical; }

         if (up && !right && !down && !left) { result(col, row) = WallType::Up; }
         if (!up && right && !down && !left) { result(col, row) = WallType::Right; }
         if (!up && !right && down && !left) { result(col, row) = WallType::Down; }
         if (!up && !right && !down && left) { result(col, row) = WallType::Left; }
      }
   }
