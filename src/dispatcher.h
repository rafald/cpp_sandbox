 ...
 */
template <typename C>
class OptimisticDispatcher final {
  private:
  ...
  public:
   ...
   invoke(A &&... args) {
      ...
      for (iter = this->callbacks.begin(); iter != this->callbacks.end(); ++iter) {
         if (auto callback = iter->lock()) { (*callback)(std::forward<A>(args)...); }
      }
      ...
      if (0 == this->concurrent_dispatcher_count) {
         ...
         this->callbacks.erase(
             std::remove_if(
                 this->callbacks.begin(), this->callbacks.end(), [](std::weak_ptr<C> callback) { return callback.expired(); }),
             this->callbacks.end());
