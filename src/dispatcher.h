...
public:
  ...
  template <typename... A> void invoke(A &&... args) {
    ...
    if (0 == this->concurrent_dispatcher_count) {
      std::cout << "callback count before erase " << callbacks.size()
                << std::endl;
      ...
      std::cout << "callback count after erase " << callbacks.size()
                << std::endl;
    }
  }
};
