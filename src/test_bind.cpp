...
template <> struct is_placeholder<My_2> : public integral_constant<int, 2> {};
}
...
namespace std {
template <> struct is_placeholder<My_1> : public integral_constant<int, 1> {};
}
...
int main() {
  int const ignored_int = 98765;
  {
    ...
    std::bind(f, _1, _2)(
        1, 2, 3, 4, 5,
        6); // but surplus args are accepted in eval call on result of bind!
    std::bind(sf, _1, _2)(
        1, 2, 3, 4, 5,
        6); // but surplus args are accepted in eval call on result of bind!
  ...
  auto b =
      std::bind(f, my_2, 2); // _1 not present so it is ignored in functor b
  ...
  auto const b_f3_x_1_2 =
      std::bind(f3, 3, std::placeholders::_1, std::placeholders::_2);
  std::cout
      << "Adding 2 to 10 selected with a custom placeholder gives "
      ...
      << '\n'
      << std::bind(f, 2, b_2)(
             ignored_int, 4) // bind expression can substitute _2 placeholder
      << '\n'
      << std::bind(f, 2, b_f3_x_1_2)(
             4, 5) // bind expression can substitute _1 placeholder
      << '\n'
      << std::is_placeholder<
             typename std::decay<decltype(b_f3_x_1_2)>::type>::value
      << '\n'
      << std::bind(f, 2, b_f3_x_1_2)(
             4, 5) // bind expression can substitute _1 placeholder
      << '\n';
