...
// _1 can be ommited in bind and only _2 provide, then first arg of eval call is ignored
// When we bind we have to specify ALL arguments, some can be values and some placeholders
///////////////////////////////////////////////////////////////////////////////
...
//    std::bind(f,_1,_2, 23423)(1,2,3,4,5,6); // bind itself can NOT receive surplus args
...
//    std::bind(sf,_1,_2, 33)(1,2); // but surplus args are NOT accepted if result of bind is used inside another bind
    }
    ...
    // (2 is binded to 2nd arg of f and my_2 becomes new 2nd arg of result of bind)
    std::cout << "Adding 2 to 11 selected with a custom placeholder gives " 
             // the first argument, namely 10, is ignored, but can not be skipped in final eval call
              << b(10, 11) 
              << '\n'
              << b(10, 11, 1242342) 
              << '\n'
...
//       << std::bind(f, 2, std::bind(f, 2, std::placeholders::_1), 2 )
       << std::bind(f, 2, b_1 )(4) // bind expression can substitute _1 placeholder
       << '\n'
       ...
       << '\n'
       << std::is_placeholder< typename std::decay< 
             decltype( b_f3_x_1_2 )
          >::type >::value
       << '\n'
       << std::bind(f, 2, b_f3_x_1_2 )(4, 5) // bind expression can substitute _1 placeholder
